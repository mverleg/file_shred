...
pub fn write_output_file(config: &EncryptConfig, file: &FileInfo, secret: &Vec<u8>, header: &Header) -> FedResult<()> {
    ...
    if file.out_pth.exists() {
        if config.overwrite() {
            ...
            return Err(format!("While encrypting, a file appeared in previously empty \
